...
def predict0(sess, text):
    """
    输入采用torch.Tensor转numpy数组
    """
    ids, seq_len, mask = pred.build_predict_text(text)
...
def predict(sess, text):
    ids, seq_len, mask = build_predict_text1(text)
...
if __name__ == '__main__':
    ...
    #batch_predict(ts, lambda t: predict0(cuda_ses, t), 'ONNX_CUDA_tensor_to_numpy')
